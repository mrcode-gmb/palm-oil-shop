...
class InventoryController extends Controller
{
    ...
    {
        $getProduct = Product::where("id", $product)->get();
        $getProduct->load(['purchases.user', 'sales.user']);
        
        return $getProduct->purchases
        // Get stock movement history
        $stockMovements = collect();

        // Add purchases as positive movements
        foreach ($getProduct->purchases as $purchase) {
            $stockMovements->push([
                'type' => 'purchase',
                'date' => $purchase->purchase_date,
                'quantity' => $purchase->quantity,
                'description' => "Purchase from {$purchase->supplier_name}",
        ...
        foreach ($getProduct->sales as $sale) {
            $stockMovements->push([
                'type' => 'sale',
                'date' => $sale->sale_date,

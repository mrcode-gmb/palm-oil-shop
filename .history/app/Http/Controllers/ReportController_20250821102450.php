...
class ReportController extends Controller
{
    ...
    public function exportExpensivePDF(Request $request)
    {
        ...
        foreach ($expenses as $expens) {
            $pdf->Cell(20, 8, $expens->created_at->format('M d'), 1, 0, 'C');
            $pdf->Cell(50, 8, substr($expens->name, 0, 25), 1, 0, 'L');
            $pdf->Cell(40, 8, '₦' . number_format($expens->amount), 1, 0, 'R');
            $pdf->Cell(25, 8, '₦' . number_format($expens->today_net_profit), 1, 0, 'R');
        }

...
class ReportController extends Controller
{
    ...
    public function exportExpensivePDF(Request $request)
    {
        ...
        $expensis = Expenses::with(['user'])
            ->whereDate('created_at', '>=', $startDate)
            ->whereDate('created_at', '<=', $endDate)
            ->orderBy('created_at', 'desc')
            ->get();

        $totalSales = $expensis->sum('total_amount');
        $totalProfit = $expensis->sum('profit');
        ...
        $pdf->Cell(0, 10, 'Summary', 0, 1, 'L');
        $pdf->SetFont('helvetica', '', 10);
        $pdf->Cell(0, 8, "Total Expenses: ₦" . number_format($totalSales, 2), 0, 1, 'L');
        $pdf->Cell(0, 8, "Number of Expenses: " . $expensis->count(), 0, 1, 'L');
        $pdf->Ln(10);

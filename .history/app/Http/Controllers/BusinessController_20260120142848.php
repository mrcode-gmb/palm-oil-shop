...
class BusinessController extends Controller
{
    ...
    private function createPurchaseHistory(Business $business)
    {
        return $business->purchases->map(function($purchase){
            ...
            $assignedInventory = $purchase->assignProduct->map(function($assignment){
                return $inventoryRemain +  (int)$assignment->assigned_quantity - (int)$assignment->returned_quantity;
            });
            return $inventoryRemain;
        });
        return $business->purchases->map(function ($product) {
            $totalPurchased = (int) $product->sum('quantity');

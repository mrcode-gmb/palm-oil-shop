...
class ReportController extends Controller
{
    ...
    public function exportExpensivePDF(Request $request)
    {
        ...
        $pdf->SetFont('helvetica', 'B', 8);
        $pdf->Cell(20, 8, 'Date', 1, 0, 'C');
        $pdf->Cell(50, 8, 'Expenses Title', 1, 0, 'C');
        $pdf->Cell(40, 8, 'Expenses Amount', 1, 0, 'C');
        $pdf->Cell(25, 8, 'Net Profit', 1, 0, 'C');

        // Table data
        $pdf->SetFont('helvetica', '', 8);
        foreach ($expenses as $expense) {
            $pdf->Cell(20, 8, $expense->created_at->format('M d'), 1, 0, 'C');
            $pdf->Cell(50, 8, substr($expense->name, 0, 25), 1, 0, 'L');
            $pdf->Cell(40, 8, '₦' . number_format($expense->amount), 1, 0, 'R');
            $pdf->Cell(25, 8, '₦' . number_format($expense->today_net_profit), 1, 0, 'R');
        }

                        ...
                        <select id="product_id" name="product_id" required class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500">
                            ...
                            @foreach($products as $product)
                                <option value="{{ $product->id }}" 
                                        ...
                                        data-unit="{{ $product->pr->unit_type }}"
                                        {{ old('product_id') == $product->id ? 'selected' : '' }}>
                                    {{ $product->product->name }} ({{ number_format($product->quantity, 1) }} {{ $product->unit_type }}s available) - ₦{{ number_format($product->selling_price, 2) }}
                                </option>
                            @endforeach
                        </select>
                        @error('product_id')

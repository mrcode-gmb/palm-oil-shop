            ...
            <div class="p-6">
                <form method="POST" action="{{ route('sales.store') }}" class="space-y-6">
                    ...
                    <div>
                        ...
                        <select id="product_id" name="product_id" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500">
                            ...
                            @foreach($products as $product)
                                <option value="{{ $product->id }}" 
                                    ...
                                    {{ $product->name }} ({{ number_format($product->current_stock, 1) }} {{ $product->unit_type }}s available) - ₦{{ number_format($product->selling_price, 2) }}
                                </option>
                            @endforeach
                        </select>
                        @error('product_id')

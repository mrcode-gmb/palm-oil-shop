        ...
        <div class="flex justify-between items-center">
            ...
            <div class="flex space-x-2">
                @if(auth()->user()->isAdmin())
                    <a href="{{ route('inventory.edit', ['inventory' => $product->id]) }}" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-md text-sm font-medium transition-colors duration-200">
                        Edit Product
                    </a>
                @endif
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <dl class="space-y-3">
                            ...
                            <div>
                                ...
                                <dd class="text-lg font-semibold text-green-600">₦{{ number_format($product->selling_price, 2) }}</dd>
                            </div>
                            ...
                            <div>
                                ...
                                <dd class="text-sm text-gray-900">{{ $product->created_at->format('M d, Y \a\t g:i A') }}</dd>
                            </div>
                        </dl>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="space-y-4">
                            <div class="bg-gray-50 p-4 rounded-lg">
                                <div class="flex items-center justify-between">
                                    <div>
                                        ...
                                        <p class="text-3xl font-bold {{ $product->current_stock == 0 ? 'text-red-600' : ($product->current_stock < 10 ? 'text-yellow-600' : 'text-green-600') }}">
                            ...
                            <!-- Stock Value -->
                            @php
                                $averageCostPrice = $product->purchases()->avg('cost_price_per_unit') ?? 0;
                                $stockValue = $product->current_stock * $averageCostPrice;
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                                <div class="text-center p-3 bg-blue-50 rounded-lg">
                                    ...
                                    <p class="text-lg font-semibold text-blue-800">₦{{ number_format($stockValue, 2) }}</p>
                                </div>
                                <div class="text-center p-3 bg-green-50 rounded-lg">
                                    <p class="text-xs text-green-600">Potential Revenue</p>
                                    <p class="text-lg font-semibold text-green-800">₦{{ number_format($potentialRevenue, 2) }}</p>
                                </div>
                                <div class="text-center p-3 bg-purple-50 rounded-lg">
                                    <p class="text-xs text-purple-600">Potential Profit</p>
                                    <p class="text-lg font-semibold text-purple-800">₦{{ number_format($potentialProfit, 2) }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        <div class="bg-white overflow-hidden shadow-sm rounded-lg">
            <div class="p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    @if(auth()->user()->isAdmin())
                        <a href="{{ route('inventory.edit', $product) }}" class="bg-purple-600 hover:bg-purple-700 text-white px-4 py-3 rounded-md text-center font-medium transition-colors duration-200">
                            <svg class="w-5 h-5 inline-block mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <button onclick="window.print()" class="bg-gray-600 hover:bg-gray-700 text-white px-4 py-3 rounded-md text-center font-medium transition-colors duration-200">
                        <svg class="w-5 h-5 inline-block mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        @if(auth()->user()->isAdmin())
            <div class="bg-white overflow-hidden shadow-sm rounded-lg">
                <div class="p-6">
                    ...
                    <form method="POST" action="{{ route('inventory.adjust-stock', $product) }}" class="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
                        @csrf
                        <div>
                            <x-input-label for="adjustment" :value="__('Adjustment Amount')" />
                            <x-text-input id="adjustment" 
                                          name="adjustment" 
                                          type="number" 
                                          step="0.1" 

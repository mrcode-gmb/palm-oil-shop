        ...
        <div class="bg-white overflow-hidden shadow-sm rounded-lg">
            <div class="p-6">
                <form method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    @if (auth()->user()->isAdmin())
                        <div>
                            ...
                            <select id="unit_type" name="unit_type"
                                ...
                                <option value="Customize" {{ request('unit_type') == 'Customize' ? 'selected' : '' }}>
                                    Customize</option>
                                <option value="Uncustomize"
                                    {{ request('unit_type') == 'Uncustomize' ? 'selected' : '' }}>Uncustomize</option>
                            </select>
                        </div>

                        <div>
                            <label for="stock_filter" class="block text-sm font-medium text-gray-700">Stock
                                Level</label>
                            <select id="stock_filter" name="stock_filter"
                                class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500">
                                <option value="">All Stock Levels</option>
                                <option value="available"
                                    {{ request('stock_filter') == 'available' ? 'selected' : '' }}>Available (> 0)
                                </option>
                                <option value="low" {{ request('stock_filter') == 'low' ? 'selected' : '' }}>Low
                                    Stock (< 10)</option>
                    ...
                    @else
                        <div>
                            <label for="status_filter" class="block text-sm font-medium text-gray-700">Assignment
                                Status</label>
                            <select id="status_filter" name="status_filter"
                                class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500">
                                <option value="">Active Assignments</option>
                                <option value="assigned" {{ request('status_filter') == 'assigned' ? 'selected' : '' }}>
                                    Assigned</option>
                                <option value="in_progress"
                                    {{ request('status_filter') == 'in_progress' ? 'selected' : '' }}>In Progress
                                </option>
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                    ...
                    <div class="bg-white overflow-hidden shadow-sm rounded-lg">
                        <div class="p-6 text-center">
                            ...
                            <p class="text-2xl font-bold text-green-600">{{ $assignments->sum('assigned_quantity') }}
                            </p>
                        </div>
                    </div>
                    <div class="bg-white overflow-hidden shadow-sm rounded-lg">
        ...
        @if (auth()->user()->isAdmin())
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                @forelse($products as $product)
                    <div
                        class="bg-white overflow-hidden shadow-sm rounded-lg {{ $product->quantity == 0 ? 'border-l-4 border-red-500' : ($product->quantity < 10 ? 'border-l-4 border-yellow-500' : 'border-l-4 border-green-500') }}">
                        <div class="p-6">
                            <!-- Product Header -->
                            <div class="flex justify-between items-start mb-4">
                                <div>
                                    <h3 class="text-lg font-medium text-gray-900">{{ $product->product->name }} <br> -
                                        ₦{{ $product->purchase_price }}</h3>
                                    <p class="text-sm text-gray-500 capitalize">{{ $product->product->unit_type }}</p>
                                </div>
                                <div class="text-right">
                                    <p
                                        class="text-lg font-bold {{ $product->quantity == 0 ? 'text-red-600' : ($product->quantity < 10 ? 'text-yellow-600' : 'text-green-600') }}">
                            ...
                            <div class="space-y-2 mb-4">
                                <div class="flex justify-between">
                                    ...
                                    <span
                                        class="text-sm font-medium">₦{{ number_format($product->purchase_price, 2) }}</span>
                                </div>
                                @if ($product->notes)
                            ...
                            <div class="mb-4">
                                ...
                                @elseif($product->quantity < $product->product->low_stock_threshold)
                                    <span
                            ...
                            <div class="flex space-x-2">
                                <a href="{{ route('inventory.show', parameters: $product) }}"
                            ...
                            @if (auth()->user()->isAdmin())
                                <div class="mt-3 pt-3 border-t border-gray-200">
                                    <div class="flex space-x-2 text-sm">
                                        <a href="{{ route('purchases.restock') }}?product={{ $product->id }}"
        ...
        @else
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                @forelse($assignments as $assignment)
                    <div
                        class="bg-white overflow-hidden shadow-sm rounded-lg {{ $assignment->isOverdue() ? 'border-l-4 border-red-500' : 'border-l-4 border-blue-500' }}">
                        <div class="p-6">
                            <!-- Assignment Header -->
                            <div class="flex justify-between items-start mb-4">
                                <div>
                                    <h3 class="text-lg font-medium text-gray-900">
                                        {{ $assignment->purchase->product->name }}</h3>
                                    <p class="text-sm text-gray-500">Expected Price:
                                        ₦{{ number_format($assignment->expected_selling_price, 2) }}/unit</p>
                                </div>
                                <div class="text-right">
                                    <span
                            ...
                            <div class="space-y-2 mb-4">
                                ...
                                <div class="flex justify-between text-sm">
                                    ...
                                    <span class="font-medium text-green-600">{{ $assignment->sold_quantity }}
                                        units</span>
                                </div>
                                <div class="flex justify-between text-sm">
                                    <span class="text-gray-600">Remaining:</span>
                                    <span class="font-medium text-orange-600">{{ $assignment->remaining_quantity }}
                                        units</span>
                                </div>
                                <div class="flex justify-between text-sm">
                                    <span class="text-gray-600">Sales Revenue:</span>
                                    <span
                                        class="font-medium text-purple-600">₦{{ number_format($assignment->actual_total_sales, 2) }}</span>
                                </div>
                            </div>
                ...
                @empty
                    <div class="md:col-span-3 bg-white overflow-hidden shadow-sm rounded-lg">
                        <div class="p-12 text-center">
                            ...
                            <p class="mt-1 text-sm text-gray-500">Contact your administrator for product assignments.
                            </p>
                        </div>
                    </div>
                @endforelse

                ...
                <div class="bg-white overflow-hidden shadow-sm rounded-lg {{ $product->current_stock == 0 ? 'border-l-4 border-red-500' : ($product->current_stock < 10 ? 'border-l-4 border-yellow-500' : 'border-l-4 border-green-500') }}">
                    <div class="p-6">
                        ...
                        <div class="flex space-x-2">
                            <a href="{{ route('inventory.show', ['inventory' => $product->id]) }}" class="flex-1 bg-blue-600 hover:bg-blue-700 text-white text-center py-2 px-3 rounded-md text-sm font-medium transition-colors duration-200">
                                View Details
                            </a>
                            @if(auth()->user()->isAdmin())

                ...
                <form method="POST" action="{{ route('inventory.update', $product) }}" class="space-y-6">
                    ...
                    <div>
                        ...
                        <select id="unit_type" 
                            ...
                            <option value="CustomizeCustomize
Uncustomize" {{ old('unit_type', $product->unit_type) == 'Customize
Uncustomize' ? 'selected' : '' }}>Litre</option>
                            <option value="UncustomizeCustomize
Uncustomize" {{ old('unit_type', $product->unit_type) == 'Customize
Uncustomize' ? 'selected' : '' }}>Jerrycan</option>
                        </select>
                    ...
                    <div class="flex items-center justify-between pt-6 border-t border-gray-200">
                        ...
                        @if($product->purchase->count() == 0 && $product->purchase->sales->count() == 0)
                            <form method="POST" action="{{ route('inventory.destroy', $product->product) }}" class="inline" onsubmit="return confirm('Are you sure you want to delete this product? This action cannot be undone.')">
                                @csrf
                                @method('DELETE')

                    ...
                    <div class="flex items-center">
                        ...
                        <div class="ml-5 w-0 flex-1">
                            <dl>
                                ...
                                <dd class="text-lg font-medium text-gray-900">₦{{ number_format($todaySellersProfit, 2) }}</dd>
                            </dl>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="flex items-center">
                        ...
                        <div class="ml-5 w-0 flex-1">
                            <dl>
                                ...
                                <dd class="text-lg font-medium text-gray-900">{{ number_format($totalStock, 1) }} units</dd>
                            </dl>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="flex items-center">
                        ...
                        <div class="ml-5 w-0 flex-1">
                            <dl>
                                ...
                                <dd class="text-lg font-medium text-gray-900">{{ $lowStockProducts->count() }} items</dd>
                            </dl>
                        </div>
                    </div>
                </div>
                        ...
                        @forelse($recentSales as $sale)
                            <div class="flex justify-between items-center py-2 border-b border-gray-100 last:border-b-0">
                                <div>
                                    ...
                                    <p class="text-xs text-gray-500">{{ $sale->user->name }} • {{ $sale->sale_date->format('M d, Y') }}</p>
                                </div>
            ...
            <div class="bg-white overflow-hidden shadow-sm rounded-lg">
                <div class="p-6">
                    <div class="flex justify-between items-center mb-4">
                        ...
                        <a href="{{ route('purchases.index') }}" class="text-sm text-blue-600 hover:text-blue-500">View all</a>
                    </div>
                    <div class="space-y-3">
                        @forelse($recentPurchases as $purchase)
                            <div class="flex justify-between items-center py-2 border-b border-gray-100 last:border-b-0">
                                <div>
                                    ...
                                    <p class="text-xs text-gray-500">{{ $purchase->supplier_name }} • {{ $purchase->purchase_date->format('M d, Y') }}</p>
                                </div>
                                <div class="text-right">
                                    <p class="text-sm font-medium text-blue-600">₦{{ number_format($purchase->total_cost, 2) }}</p>
                                    <p class="text-xs text-gray-500">{{ number_format($purchase->quantity, 1) }} units</p>
                                </div>
                            </div>
                        @empty
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        @foreach($lowStockProducts as $product)
                            <div class="bg-red-50 p-3 rounded-md">
                                ...
                                <p class="text-xs text-red-600">Only {{ number_format($product->current_stock, 1) }} units left</p>
                                <a href="{{ route('purchases.create') }}" class="text-xs text-red-700 hover:text-red-600 underline">Restock now</a>
                            </div>
                        @endforeach
                    </div>
                </div>

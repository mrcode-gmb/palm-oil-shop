                ...
                <form method="POST" action="{{ route('purchases.store') }}" class="space-y-6">
                    ...
                    <div>
                        ...
                        <select id="product_id" name="product_id" required
                            ...
                            @foreach ($ as $purchased)
                                <option value="{{ $purchased->id }}" data-unit="{{ $purchased->unit_type }}"
                                    data-current-stock="{{ $purchased->current_stock }}"
                                    {{ old('product_id') == $purchased->id ? 'selected' : '' }}>
                                    {{ $purchased->product->name }} ({{ ucfirst($purchased->product->unit_type) }}) - Current Stock:
                                    {{ number_format($purchased->product->quantity, 1) }}
                                </option>
                            @endforeach
                        </select>
                        @error('product_id')

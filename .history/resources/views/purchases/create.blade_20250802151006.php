<x-shop-layout>
            ...
            <div class="p-6">
                <form method="POST" action="{{ route('purchases.store') }}" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div>
                            <label for="cost_price_per_unit" class="block text-sm font-medium text-gray-700">Cost Price Per Unit (₦)</label>
                            <input type="number" 
                                   id="cost_price_per_unit" 
                                   name="cost_price_per_unit" 
                                   step="0.01" 
                                   min="0.01" 
                                   value="{{ old('cost_price_per_unit') }}" 
                                   required 
                                   class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500">
                            @error('cost_price_per_unit')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const costPriceInput = document.getElementById('cost_price_per_unit');

                    ...
                    <div>
                        ...
                        <select id="product_id" name="product_id" required
                            ...
                            @foreach ($products as $product)
                                <option value="{{ $product->products->id }}" data-unit="{{ $product->product->unit_type }}"
                                    data-current-stock="{{ $product->quantity }}"
                                    {{ old('product_id') == $product->id ? 'selected' : '' }}>
                                    {{ $product->product->name }} ({{ ucfirst($product->product->unit_type) }}) - Current Stock:
                                    {{ number_format($product->quanity, 1) }}
                                </option>
                            @endforeach
                        </select>
                        @error('product_id')
